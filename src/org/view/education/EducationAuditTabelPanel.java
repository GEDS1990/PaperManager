...
public class EducationAuditTabelPanel extends JPanel implements ActionListener{
	...
	public EducationAuditTabelPanel(Education  education)
	{  
		eduNo=education.getEducationNo();
		eduName=education.getEducationName();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==flushButton)
		{
			...
			data=paperService.getRecord_Edu("select * "
					...
					+ "LEFT JOIN zhuanjia ON paper.Z_no=zhuanjia.Z_no where paper.P_state=0  order by T_name asc");
			changeModel(data);
		}
		...
		{   
			...
			if(paper!=null)
			{
				...
				paper.setPaperNo((int)table.getModel().getValueAt(row, 0));
				int state=new EduAuditPaperDialog(paper).getI();//判断返回数据
				if(state==11||state==00){
