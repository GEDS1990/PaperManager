...
public class MyrelesePaperPanel  extends JPanel implements ActionListener{
	private static final long serialVersionUID = 1L;

	MyrelesePaperTabelPanel tabelPanel;
	private JButton textButton;
	private JPanel condPanel;
	private JTextField textField;
	private int teacherNo;
	
	public MyrelesePaperPanel(Teacher teacher)
	{
		...
		tabelPanel=new MyrelesePaperTabelPanel(teacher);
	...
	public void actionPerformed(ActionEvent e) {
		String sql="select * "
				...
				+ "LEFT JOIN zhuanjia ON paper.Z_no=zhuanjia.Z_no where paper.T_no="+teacherNo +" and paper.P_state=5 and";
		if(textField.getText().trim().length()!=0){
			String text=textField.getText().trim();
			sql=sql+"( P_no like '%"+text+"%' or P_name like '%"+text+"%' or T_name like '%"+text+"%' or D_name like '%"+text+"%')";
			IPaperService paperService= new PaperService();
			Object data[][];
			data=paperService.getRecord_Relese(sql);
