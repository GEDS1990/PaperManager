...
public class PasswordPanel extends JPanel implements ActionListener{
	...
	public PasswordPanel(Teacher teacher) {
			...
			if(oldPassword.length()!=0&&newPassword_1.length()!=0&&newPassword_2.length()!=0){
				if(newPassword_1.equals(newPassword_2)){
					...
					if(teacherPassword.equals(oldPassword)){
						...
						teacher.setTeacherPassword(newPassword_2);
						teacher.setTeacherNo(teacherNo);
						if(teacherService.updata_password(teacher)==true){
							JOptionPane.showMessageDialog(this, "密码修改成功", "提示", 1);
							oldField.setText(null);
							newField_1.setText(null);
							newField_2.setText(null);
						}else{
//							System.out.println(teacherPassword+"---"+oldPassword+"---"+newPassword_2);
