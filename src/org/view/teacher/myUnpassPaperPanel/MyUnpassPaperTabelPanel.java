...
import org.view.teacher.myUnauditPaperPanel.MyPaperUpdateDialog;

public class MyUnpassPaperTabelPanel extends JPanel implements ActionListener{
	private static final long serialVersionUID = 1L;
	JScrollPane  scrollPanel;
	JTable table;
	DefaultTableModel tableModel;
	Object data[][];
	Object columnNames[]={"论文编号","论文标题","教务处审核结果","教务处审核人","教务处审核时间","专家处审核结果","专家处审核人","专家处审核时间","专家处审核意见"};
	IPaperService paperService;
	JPanel buttonPanel;
	JButton allButton,flushButton,deleteButton,updateButton;
	int teacherNo;

	public MyUnpassPaperTabelPanel(Teacher teacher)
	{  
...
//			tableModel.setValueAt(paper.getMaxNumber(), row, 4);
	...
	{   
		...
		{ 
			...
			paper.setEduName((String)tableModel.getValueAt(row, 3));
			paper.setZjName((String)tableModel.getValueAt(row, 6));
			return paper;
		}
		return null;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==flushButton)
		{
			...
			data=paperService.getRecord_Unpass("select * "
					...
					+ "LEFT JOIN zhuanjia ON paper.Z_no=zhuanjia.Z_no where paper.T_no="+teacherNo +" and (paper.P_state=2 or paper.P_state=4)");
			changeModel(data);
		}
...
//					System.out.println("删了");
		...
		{   
			...
			if(paper!=null)
			{
				...
				new MyUnpassPaperInfoDialog(paper);
			}
			else
			{
